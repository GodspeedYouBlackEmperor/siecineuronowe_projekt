...
class KDDPreprocessor:
    def __init__(self, group_records=False, is_window=False, window_size=20, stride=10):
    ...
    def preprocess_train_data(self, df: pd.DataFrame, label):
        if(self.group_records):
            df['label'] = df['label'].replace(self.attacks_categories)

        df = df[df['label'] == label]
        df.reset_index(drop=True, inplace=True)
        # df.to_csv(f'Results/train0_{label}.csv')
    ...
    def preprocess_test_data(self, df: pd.DataFrame, label):
        if(self.group_records):
            df['label'] = df['label'].replace(self.attacks_categories)
